...
int main(int argc, char* argv[])
{
	...
	ofstream fout("..//kalman//result.txt");
	...
	cout << "含噪声测量";
	for (int j = 0; j < z_meas[i].rows(); ++j) {
		for (int k = 0; k < z_meas[i].cols(); ++k) {
			cout << z_meas[i](j, k) << " ";
		}
	}

	cout << "后验估计";
	for (int j = 0; j < x_evlt[i].rows(); ++j) {
		for (int k = 0; k < x_evlt[i].cols(); ++k) {
			cout << x_evlt[i](j, k) << " ";
		}
	}

	cout << "真值";
	for (int j = 0; j < x_real[i].rows(); ++j) {
		for (int k = 0; k < x_real[i].cols(); ++k) {
			cout << x_real[i](j, k) << " ";
		}
	}
